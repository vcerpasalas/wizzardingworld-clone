...
const loadedMagicalFeatures = async () => {
    try {
        const res = await fetch("https://github.com/vcerpasalas/wizzardingworld-clone/blob/develop/json/magicalFeatures.json");
        data = await res.json();
        //console.log(data);
        
        for (let index = 0; index < data.length; index++) {
            if(index<=1){
                renderMagicalFeaturesFirst(data[index].src, data[index].alt, data[index].href, data[index].paragraph, magicalFeaturesFeaturesFirst);
            }
            else{
                renderMagicalFeaturesSecond(data[index].src, data[index].alt, data[index].href, data[index].paragraph, magicalFeaturesFeaturesSecond);
            }
        }
